...
``` Java
    DesktopOptions options = new DesktopOptions()
                    ...
                    // Ожидание на запуск приложения (по умолчанию 2 сек)
                    .setLaunchDelay(Integer launchDelay)
                    // Подключение к процессу по имени
                    .setProcessName(String processName)
                    // Время поиска процесса (по умолчанию 30 сек)
...
`setDebugConnectToRunningApp()` - Если false (по умолчанию) - всегда запускается новый процесс приложения (с закрытием текущего, если имеется). Если true и приложение не запущено, то запускает приложение. Если true и приложение запущено - то просто использует текущее состояние приложения. Так же если true - то приложение не закрывается по завершении сессии.
